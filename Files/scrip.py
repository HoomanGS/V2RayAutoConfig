...
def generate_simple_readme(protocol_counts, country_counts, all_keywords_data, github_repo_path="tahmaseb73/V2RayAutoConfig", github_branch="main"):
    tz = pytz.timezone('Asia/Tehran')
    now = datetime.now(tz)
    timestamp = now.strftime("%Y-%m-%d %H:%M:%S %Z")

    raw_github_base_url = f"https://raw.githubusercontent.com/{github_repo_path}/refs/heads/{github_branch}/{OUTPUT_DIR}"  # تغییر به configs
...
async def main():
    ...
    generate_simple_readme(protocol_counts, country_counts, categories_data, 
                          github_repo_path="tahmaseb73/V2RayAutoConfig",
                          github_branch="main")
